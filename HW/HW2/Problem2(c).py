...
reservation = PriorityQueue(1000)
while 1 > 0:
    info = input("input name and priority\nin form of (Name, int) or done\n")
    if info == "done":
        break
    else:
        name = info[2:-4]
        priority = info[-2:-1]
        info_tuple = (name, priority)
        reservation.Push(info_tuple)
for i in range(len(reservation.pq)):
    print(reservation.pq[i], end = '')
print("\n")
